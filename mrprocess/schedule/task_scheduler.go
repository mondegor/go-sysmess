...
var (
	...
	ErrInternalNoTasks = errors.New("no tasks to start for the task scheduler")

	// ErrInternalZeroParam - ошибка при нулевых period и timeout задачи (attrs: param_name, task_name).
	ErrInternalZeroParam = errors.New("task has zero param for the task scheduler")
)
...
func (p *TaskScheduler) Start(ctx context.Context, ready func()) error {
	...
	for i := range p.tasks {
		...
		go func(ctx context.Context, task mrprocess.Task) {
...
func (p *TaskScheduler) startup(ctx context.Context) error {
	if len(p.tasks) == 0 {
		return fmt.Errorf("%w [scheduler_name=%s]", ErrInternalNoTasks, p.caption)
	}

	// запуск задач на этапе старта планировщика выполняется последовательно
	for _, task := range p.tasks {
		if task.Period() == 0 {
			return fmt.Errorf("%w [param_name=period, task_name=%s]", ErrInternalZeroParam, task.Caption())
		}

		if task.Timeout() == 0 {
			return fmt.Errorf("%w [param_name=timeout, task_name=%s]", ErrInternalZeroParam, task.Caption())
		}
