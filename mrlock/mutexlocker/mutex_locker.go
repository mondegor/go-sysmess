...
func New(logger mrlog.Logger, tracer mrtrace.Tracer, minBufferSize int) *Locker {
	return &Locker{
		tracer: tracer,
		logger: logger,
...
func (l *Locker) LockWithExpiry(ctx context.Context, key string, expiry time.Duration) (unlock func(), err error) {
	...
	if exp, ok := l.keys[key]; ok && exp > time.Now().UnixNano() {
		return nil, mr.ErrStorageLockKeyNotCaptured.New("source", mutexLockerName, "key", key)
	}
	...
	return func() {
		...
		if _, ok := l.keys[key]; ok {
			...
			l.logger.Warn(ctx, "unlock", "error", mr.ErrStorageLockKeyNotHeld.New("source", mutexLockerName, "key", key))
		}
	}, nil
}
