...
- Генерация ID ошибки вынесена в отдельный объект с возможностью переопределения.
  Для этого добавлена глобальная переменная `mrerr.GlobalIDGenerator`;
- Добавлена глобальная переменная `mrerr.PrepareErrorTypeFunc` для формирования
  типов ошибок на основе указанных для них опций.
