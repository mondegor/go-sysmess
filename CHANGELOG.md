...
- Переработан пакет ошибок:
  - объект AppErrorFactory заменён на `AppErrorProto` который теперь сам является ошибкой;
  - избавление от глобальных переменных;
- Написаны тесты для пакета ошибок;
- Изменился интерфейс `CustomError`;
- Добавлены примеры работы с пакетом ошибок;
